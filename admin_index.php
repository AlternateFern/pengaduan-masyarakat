...
$query = $koneksi->query("SELECT * FROM pengaduan JOIN masyarakat ON masyarakat.nik = pengaduan.nik");
$data  = $query->fetchAll();
...
<head>
  ...
  <title>Home (ADMIN) | Pengaduan Masyarakat</title>
    ...
    <table class="table" style="margin-top:20px;">
      <tbody>
          <tr>
            ...
            <td><svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="25.000000pt" height="25.000000pt" viewBox="0 0 15.000000 38.000000" preserveAspectRatio="xMidYMid meet">
            ...
            <td class="w-30"><?= $pengaduan['nik'] ?> / <?= $pengaduan['username'] ?></td>
            <td class="w-30"><?= $pengaduan['tgl_pengaduan'] ?></td>
            <td class="w-30">
            ...
            <a href="proses/proses_hapus.php?id_pengaduan=<?= $pengaduan['id_pengaduan']; ?>" class="btn btn-sm btn-danger ml-auto" style="padding: 4px 13px;" onclick="return confirm('Konfirmasi Penghapusan Data?')">Hapus</a> -->
              <a href="admin_info_pengaduan.php?id_pengaduan=<?= $pengaduan['id_pengaduan']; ?>" class="btn btn-sm btn-success ml-auto" style="padding: 4px 20px; margin-right: 3px;">Buka</a>
            </td>
          </tr>
      </tbody>
    </table>
